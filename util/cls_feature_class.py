...
class FeatureClass:
    def __init__(self, params, is_eval=False):
        ...
        # data set
        # param['dataset']={foa, mic}
        self._dataset_combination = '{}_{}_{}'.format(params['dataset'], 'eval' if is_eval else 'dev',
                                                      params['kind_data'])
        # path to audio files
        self._aud_dir = os.path.join(self._dataset_dir, self._dataset_combination)
        # path to meta files, ground truth
        self._desc_dir = None if is_eval else os.path.join(self._dataset_dir, 'metadata_dev_' + params['kind_data'])
    ...
    def _get_spectrogram_for_file(self, audio_filename, is_permutation=False):
        audio_in, fs = self._load_audio(os.path.join(self._aud_dir, audio_filename))
        # Random permutation channels of the origin audio file
        if is_permutation:
            audio_in = self.switchChannel(audio_in)
            print(audio_filename, 'is switched!')
    ...
    @staticmethod
    def switchChannel(audio, permutation=None):
        """
        :param permutation: a tuple or list that is the new order of channels, if value==None -> create a
            random permutation
        :param audio: audio is an matrix with 4 columns
        :return: audio matrix with switched channel as permutation
        """
        if permutation is None:
            permutation = np.random.permutation(audio.shape[-1])

        audio_switched = np.zeros_like(audio)
        for index, c in enumerate(permutation):
            audio_switched[:, index] = audio[:, c]
        return audio_switched

    # ------------------------------- EXTRACT FEATURE AND PREPROCESS IT -------------------------------
    def extract_all_feature(self, is_permutation=False):
        # setting up folders
        ...
        for file_cnt, file_name in enumerate(os.listdir(self._aud_dir)):
            ...
            spect = self._get_spectrogram_for_file(wav_filename, is_permutation)
    ...
    def extract_all_labels(self):
        ...
        for file_cnt, file_name in enumerate(os.listdir(self._desc_dir)):
            # TODO this is not required if the dataset is clean(or when the mix number is larger due to generating)
            # if len(file_name) != 26:  # checking clean metadata files
            #     continue
            wav_filename = '{}.wav'.format(file_name.split('.')[0])
    ...
    def write_output_format_file(self, _output_format_file, _output_format_dict):
        ...
        # _fid.write('{},{},{},{}\n'.format('frame number with 20ms hop (int)', 'class index (int)',
        # 'azimuth angle (int)', 'elevation angle (int)'))
