...
class SpecAugment():
    ...
    def __init__(self):
        ...
        # self.mel_spectrogram = np.reshape(self.mel_spectrogram, (-1, self.mel_spectrogram.shape[0],
    ...
    def time_warp(self, mel_spectrogram):
        ...
        random_pt = horiz_line_thru_ctr[random.randrange(self.W, tau - self.W)]  # random point along the
...
class SpecAugmentTensor:
    ...
    def freq_mask(self, mel_spectrogram):
        mel_spectrogram = tf.keras.backend.variable(mel_spectrogram)
        v = mel_spectrogram.shape[1]  # no. of mel freq channels

        # apply m_F frequency masks to the mel spectrogram
        for i in range(self.m_F):
            f = int(tf.random.uniform(shape=[], maxval=self.F, dtype=tf.int32))  # [0, F)
            f0 = random.randint(0, v - f)  # [0, v - f)

            shape = mel_spectrogram.shape
            mean = tf.constant(tf.reduce_mean(mel_spectrogram), shape=[shape[0], f, shape[2], shape[3]])
            mel_spectrogram[:, f0:f0 + f, :, :].assign(mean)

        return tf.convert_to_tensor(mel_spectrogram)
    ...
    def time_mask(self, mel_spectrogram):
        ...
        # apply m_T time masks to the mel spectrogram
        for i in range(self.m_T):
            t = int(tf.random.uniform(shape=[], maxval=self.T, dtype=tf.int32))  # [0, T)
            t0 = random.randint(0, tau - t)  # [0, tau - t)
            shape = mel_spectrogram.shape
            mean = tf.constant(tf.reduce_mean(mel_spectrogram), shape=[shape[0], shape[1], t, shape[3]])
            mel_spectrogram[:, :, t0:t0 + t, :].assign(mean)
