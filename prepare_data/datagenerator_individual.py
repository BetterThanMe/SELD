...
class DataGenerator_Individual:
    ...
    def load_data(self):
        ...
        if self._iseval is False:
            ...
            for label_dir in self._label_dir:
                file = pathlib.Path(label_dir).joinpath(self._filename.split('/')[-1])
                if file.is_file():
                    label = np.load(file.__str__())
                    break
        else:
